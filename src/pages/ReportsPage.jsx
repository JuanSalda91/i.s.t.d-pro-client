...
function RevenueTooltip({ active, payload, label }) {
  ...
  return (
    ...
    >
      ...
      {payload.map((p) => (
        <p key={p.dataKey}>
          {p.name}: <span style={{ color: "#93c5fd" }}>${Number(p.value).toFixed(2)}</span>
        </p>
      ))}
    </div>
  );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#f1f5f9' }}>
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-medium" style={{ color: '#64748b' }}>Loading reports…</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <main className="flex-1 p-8 overflow-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: "#1e293b" }}>Reports</h1>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm mb-6" style={{ border: "1px solid #e2e8f0" }}>
          ...
          {monthlyData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={monthlyData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                ...
                />
                <Tooltip content={<RevenueTooltip />} cursor={{ fill: "rgba(59,92,212,0.05)" }} />
        ...
        <div className="grid gap-6" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))" }}>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{ border: "1px solid #e2e8f0" }}>
            <h2 className="text-base font-semibold mb-1" style={{ color: "#1e293b" }}>Revenue Trend</h2>
            <p className="text-xs mb-5" style={{ color: "#94a3b8" }}>Month-over-month revenue line</p>

            {monthlyData.length === 0 ? (
              <div className="py-12 text-center text-sm" style={{ color: "#94a3b8" }}>No data</div>
            ) : (
          ...
          {/* Top Products (Pie) */}
          <div className="bg-white rounded-2xl p-6 shadow-sm" style={{ border: "1px solid #e2e8f0" }}>
            <h2 className="text-base font-semibold mb-1" style={{ color: "#1e293b" }}>Top Products</h2>
            <p className="text-xs mb-5" style={{ color: "#94a3b8" }}>By quantity sold (top 5)</p>

            {topProducts.length === 0 ? (
              <div className="py-12 text-center text-sm" style={{ color: "#94a3b8" }}>No product data</div>
            ) : (
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie
