    ...
    <div className="min-h-screen flex" style={{ background: '#f1f5f9', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <main className="flex-1 p-8 overflow-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: '#1e293b' }}>Invoices</h1>
        ...
        {/* ── FILTERS ─────────────────────────────────────────────────────── */}
        <div className="bg-white rounded-2xl px-5 py-4 mb-5 flex items-center gap-4 shadow-sm" style={{ border: '1px solid #e2e8f0' }}>
          ...
          {statusFilter && (() => {
            const filterStyles = {
              paid:      { bg: 'rgba(51,184,51,0.1)',   color: '#16a34a', dot: '#33B833' },
              cancelled: { bg: 'rgba(239,68,68,0.1)',   color: '#dc2626', dot: '#ef4444' },
              overdue:   { bg: 'rgba(234,88,12,0.1)',   color: '#c2410c', dot: '#f97316' },
              sent:      { bg: 'rgba(59,92,212,0.1)',   color: '#3B5CD4', dot: '#3B5CD4' },
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden" style={{ border: '1px solid #e2e8f0' }}>
          {visibleInvoices.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm font-medium" style={{ color: '#94a3b8' }}>No invoices found</p>
              <p className="text-xs mt-1" style={{ color: '#cbd5e1' }}>Try adjusting the filter or create an invoice from a completed sale</p>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Invoice #</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Customer</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Email</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Total</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Status</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Created</th>
                ...
                {visibleInvoices.map((inv) => {
                  ...
                  return (
                    <tr key={inv._id} style={{ borderBottom: '1px solid #f1f5f9' }}
                      ...
                      {/* Customer */}
                      <td className="px-4 py-4 text-sm font-medium" style={{ color: '#1e293b' }}>{inv.customerName}</td>

                      {/* Email */}
                      <td className="px-4 py-4 text-sm" style={{ color: '#64748b' }}>{inv.customerEmail}</td>
                      ...
                      <td className="px-4 py-4" style={{ minWidth: '200px' }}>
                        ...
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <select
                            ...
                            onChange={(e) => handleStatusSelectChange(inv._id, e.target.value)}
                          ...
                          <button
                            ...
                            style={{ background: savingStatusId === inv._id ? '#93c5fd' : '#3B5CD4' }}
                            onMouseEnter={e => { if (savingStatusId !== inv._id) e.currentTarget.style.background = '#2d4ab0'; }}
                            onMouseLeave={e => { if (savingStatusId !== inv._id) e.currentTarget.style.background = '#3B5CD4'; }}>
                        ...
                        {(editingStatus[inv._id] || inv.status) === 'paid' && (
                          <div className="flex items-center gap-2 mt-1.5 pt-1.5" style={{ borderTop: '1px dashed #e2e8f0' }}>
                            <span className="text-xs" style={{ color: '#94a3b8' }}>Payment:</span>
                            <select
                        ...
                        {inv.paymentDate && (
                          <p className="mt-1.5 text-xs flex items-center gap-1" style={{ color: '#94a3b8' }}>
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            Paid {new Date(inv.paymentDate).toLocaleDateString()} · {inv.paymentMethod || 'unknown'}
                          </p>
                        )}
                      </td>

                      {/* Created date */}
                      <td className="px-4 py-4 text-sm" style={{ color: '#64748b' }}>{formattedDate}</td>
                      ...
                      <td className="px-4 py-4 text-center">
                        <button
                          ...
                          style={{
                            background: downloadingId === inv._id ? 'rgba(51,184,51,0.07)' : 'rgba(51,184,51,0.1)',
                            color: '#16a34a',
                            border: '1px solid rgba(51,184,51,0.25)',
                          }}
                          onMouseEnter={e => { if (downloadingId !== inv._id) { e.currentTarget.style.background = 'rgba(51,184,51,0.2)'; e.currentTarget.style.borderColor = 'rgba(51,184,51,0.45)'; } }}
                          onMouseLeave={e => { if (downloadingId !== inv._id) { e.currentTarget.style.background = 'rgba(51,184,51,0.1)'; e.currentTarget.style.borderColor = 'rgba(51,184,51,0.25)'; } }}>
          ...
          <div className="mt-5 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                onMouseEnter={e => { if (pagination.currentPage > 1) { e.currentTarget.style.borderColor = '#3B5CD4'; e.currentTarget.style.color = '#3B5CD4'; } }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = '#e2e8f0'; e.currentTarget.style.color = '#475569'; }}>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all duration-150"
                style={{ border: '1px solid #e2e8f0', background: '#fff', color: '#475569' }}
                onMouseEnter={e => { if (pagination.currentPage < pagination.totalPages) { e.currentTarget.style.borderColor = '#3B5CD4'; e.currentTarget.style.color = '#3B5CD4'; } }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = '#e2e8f0'; e.currentTarget.style.color = '#475569'; }}>
                Next
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
