    ...
    <div className="min-h-screen flex" style={{ background: '#f1f5f9', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <main className="flex-1 p-8 overflow-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold" style={{ color: '#1e293b' }}>Sales</h1>
        ...
        <div className="bg-white rounded-2xl px-5 py-4 mb-5 flex items-center gap-4 shadow-sm" style={{ border: '1px solid #e2e8f0' }}>
          ...
          {statusFilter && (
            <span className="flex items-center gap-1.5 text-xs font-medium px-3 py-1 rounded-full"
              style={{
                background: statusFilter === 'completed' ? 'rgba(51,184,51,0.1)' : statusFilter === 'cancelled' ? 'rgba(239,68,68,0.1)' : 'rgba(245,158,11,0.1)',
                color: statusFilter === 'completed' ? '#16a34a' : statusFilter === 'cancelled' ? '#dc2626' : '#b45309',
              }}>
              <span className="w-1.5 h-1.5 rounded-full"
                style={{ background: statusFilter === 'completed' ? '#33B833' : statusFilter === 'cancelled' ? '#ef4444' : '#f59e0b' }} />
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden" style={{ border: '1px solid #e2e8f0' }}>
          {sales.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm font-medium" style={{ color: '#94a3b8' }}>No sales found</p>
              <p className="text-xs mt-1" style={{ color: '#cbd5e1' }}>Try adjusting the filter or create a new sale</p>
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                  <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-widest" style={{ color: '#94a3b8' }}>Date</th>
              ...
              <tbody>
                {sales.map((sale) => {
                  ...
                  // Status badge styles
                  const statusStyles = {
                    completed: { bg: 'rgba(51,184,51,0.1)',   color: '#16a34a', dot: '#33B833' },
                    cancelled: { bg: 'rgba(239,68,68,0.1)',   color: '#dc2626', dot: '#ef4444' },
                    pending:   { bg: 'rgba(245,158,11,0.1)',  color: '#b45309', dot: '#f59e0b' },
                  };
                  const badge = statusStyles[sale.status] || statusStyles.pending;
                  ...
                  return (
                    <tr key={sale._id} style={{ borderBottom: '1px solid #f1f5f9' }}
                      ...
                      {/* Date */}
                      <td className="px-6 py-4 text-sm" style={{ color: '#64748b' }}>{formattedDate}</td>

                      {/* Customer name */}
                      <td className="px-4 py-4 text-sm font-medium" style={{ color: '#1e293b' }}>{sale.customerName}</td>

                      {/* Email */}
                      <td className="px-4 py-4 text-sm" style={{ color: '#64748b' }}>{sale.customerEmail}</td>
                      ...
                      <td className="px-4 py-4">
                        ...
                        <div className="flex items-center gap-1.5">
                          <select
                            ...
                            onChange={(e) => handleSaleStatusChange(sale._id, e.target.value)}
                          ...
                          <button
                            ...
                            style={{ background: savingStatusId === sale._id ? '#93c5fd' : '#3B5CD4' }}
                            onMouseEnter={e => { if (savingStatusId !== sale._id) e.currentTarget.style.background = '#2d4ab0'; }}
                            onMouseLeave={e => { if (savingStatusId !== sale._id) e.currentTarget.style.background = '#3B5CD4'; }}>
                      ...
                      <td className="px-4 py-4 text-center">
                        <button
                          ...
                          style={{
                            background: creatingInvoiceId === sale._id ? 'rgba(58,150,212,0.1)' : 'rgba(58,150,212,0.12)',
                            color: '#3A96D4',
                            border: '1px solid rgba(58,150,212,0.25)',
                          }}
                          onMouseEnter={e => { if (creatingInvoiceId !== sale._id) { e.currentTarget.style.background = 'rgba(58,150,212,0.22)'; e.currentTarget.style.borderColor = 'rgba(58,150,212,0.5)'; } }}
                          onMouseLeave={e => { if (creatingInvoiceId !== sale._id) { e.currentTarget.style.background = 'rgba(58,150,212,0.12)'; e.currentTarget.style.borderColor = 'rgba(58,150,212,0.25)'; } }}>
